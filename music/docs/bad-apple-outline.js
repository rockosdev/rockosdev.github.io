...
Am I dreaming? Do I see nothing? Is it just useless to speak my words?
I've gotten tired of being sad; all I want is to live feeling nothing at all
If I'm given bewildering remarks, my heart is like in the clouds up above

Is there a future for me? Do I exist in a world like this?
Wondering if I'm painful or sad, I don't know what's all about me
I've gotten tired of walking anymore; I don't care what people think about me
If I can ever change myself, will my heart be able to turn white?
...
Am I dreaming? Do I see nothing? Is it just useless to speak my words?
I've gotten tired of being sad; all I want is to live feeling nothing at all
If I'm given hesitating words, my heart is like in the clouds up above

If I bring myself to make a move, if I change everything, I'll paint it all black

Is there a future for wasteful time? Do I exist in a place like this?
If you want to tell about me, if there's a word to describe me it's like "good for nothing"

Do I exist in a place like this? Do I exist in time like this?
If I can ever change myself, will my heart be able to turn white?
...
If I make a move, I'll destroy everything for once and for all
If I feel sad, will my heart be able to turn white?

I don't know about you, me and everything around me
If I open up my heavy eyelids; if I were to destroy everything, let it all turn black`;

// pages/bad-apple-outline.ts
var ZH_FONT_SIZE = 14;
var EN_FONT_SIZE = 12;
var LINE_HEIGHT = 16;
var FRAME_ANALYSIS_THRESHOLD = 128;
var MAX_DEVICE_PIXEL_RATIO = 1.5;
var ANALYSIS_SCALE = 0.5;
var START_MUTED = false;
var RANGE_SNAP = 2;
var MIN_RANGE_WIDTH = 10;
var STROKE_BLUR = 0.5;
var MAX_LAYOUT_RESETS = 8;
var START_CURSOR = { segmentIndex: 0, graphemeIndex: 0 };
var ZH_FONT = `${ZH_FONT_SIZE}px "Noto Sans CJK SC", "Noto Sans SC", "Source Han Sans SC", "PingFang SC", "Microsoft YaHei", sans-serif`;
var EN_FONT = `600 ${EN_FONT_SIZE}px "Helvetica Neue", Helvetica, Arial, sans-serif`;
var stageNode = document.getElementById("stage");
if (!(stageNode instanceof HTMLCanvasElement))
  throw new Error("#stage not found");
var videoNode = document.getElementById("source");
if (!(videoNode instanceof HTMLVideoElement))
  throw new Error("#source not found");
var stage = stageNode;
var video = videoNode;
var stageContext = stage.getContext("2d", { alpha: false });
if (stageContext === null)
  throw new Error("2d context unavailable");
var stageCtx = stageContext;
...
if (analysisContext === null)
  throw new Error("analysis 2d context unavailable");
var analysisCtx = analysisContext;
var zhTrack = createPreparedTrack(ZH_TEXT, ZH_FONT);
var enTrack = createPreparedTrack(EN_TEXT, EN_FONT);
var lastFrameSignature = "";
var lastPlacements = [];
var rafId = 0;
var videoFrameCallbackId = null;
var renderScheduled = false;
function createPreparedTrack(text, font) {
  return {
...
function nextTextSlice(track, width) {
...
function getVideoRect(viewportWidth, viewportHeight) {
...
function drawVideoFrame(ctx, rect) {
  ctx.fillStyle = "#000";
  ctx.fillRect(0, 0, stage.width, stage.height);
  ctx.drawImage(video, rect.x, rect.y, rect.width, rect.height);
}
function ensureAnalysisCanvas(width, height) {
  if (analysisCanvas.width !== width)
    analysisCanvas.width = width;
  if (analysisCanvas.height !== height)
    analysisCanvas.height = height;
}
function luminanceFromPixel(data, offset) {
  return data[offset] * 0.299 + data[offset + 1] * 0.587 + data[offset + 2] * 0.114;
}
function snapRange(value) {
...
function scanFrame(rect) {
  ...
  for (let y = Math.floor(scaledLineHeight / 2);y < analysisHeight; y += scaledLineHeight) {
    ...
    if (active) {
      pushRange(currentWhite ? zhRanges : enRanges, rect.x + currentStart * scaleX, rect.x + analysisWidth * scaleX);
    }
    if (zhRanges.length > 0)
      zh.push({ y: rect.y + y * scaleY, ranges: zhRanges });
    if (enRanges.length > 0)
      en.push({ y: rect.y + y * scaleY, ranges: enRanges });
  }
  return {
    zh,
    en,
...
function layoutRows(rows, track, color) {
  const placements = [];
  for (let rowIndex = 0;rowIndex < rows.length; rowIndex++) {
    const row = rows[rowIndex];
    for (let rangeIndex = 0;rangeIndex < row.ranges.length; rangeIndex++) {
      ...
      if (availableWidth < MIN_RANGE_WIDTH)
        continue;
...
function computePlacements(containers) {
  resetTrack(zhTrack);
  resetTrack(enTrack);
  return [
...
function drawPlacements(ctx, placements) {
